...
export default function OwnerDashboard() {
  const { stall, owner, logout, setStall } = useOwnerAuth();
  const isManager = owner?.staff_role === 'manager';
      ...
      <header className="border-b-2 border-ink bg-paper px-6 py-5 md:px-10">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            {isManager && (
              <Link to="/owner/staff" className="border-2 border-ink px-3 py-1.5 text-sm font-medium hover:bg-ink hover:text-paper transition-colors">
                Staff
              </Link>
            )}
            <NotificationBell fetchNotifications={api.getOwnerNotifications} markAllRead={api.markOwnerNotificationsRead} />
